...
def arg_parse():
    ...
    parser.add_argument("--cfg", dest = 'cfgfile', help = 
                        "Config file",
                        default = "utils/cfg/yolov3.cfg", type = str)
    parser.add_argument("--weights", dest = 'weightsfile', help = 
                        "weightsfile",
                        default = "utils/yolov3.weights", type = str)
...
#     assert inp_dim > 32
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            classes = load_classes('utils/data/coco.names')
            colors = pkl.load(open("utils/pallete", "rb"))
            
            list(map(lambda x: write(x, orig_im), output))
            empty = output.size(0)-3
